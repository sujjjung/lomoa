...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    if (apiKey) {
      ...
      const mainCharFromServer = apiResponse.data.find(c => c.CharacterName === mainCharacter);
      if (!mainCharFromServer) throw new Error('대표 캐릭터를 원정대에서 찾을 수 없습니다.');
      
      const serverName = mainCharFromServer.ServerName;
      const sameServerChars = apiResponse.data.filter(c => c.ServerName === serverName);

      const [allRaids] = await connection.query('SELECT id, entry_level, gates, gold_per_gate FROM raids');

      for (const char of sameServerChars) {
        ...
        const [charInsertResult] = await connection.query(
          ...
          [ newUserId, char.CharacterName, itemLevel, char.CharacterClassName, `https://placehold.co/48x48/cccccc/ffffff?text=${char.CharacterName.charAt(0)}`, itemLevel >= 1600 ]
        );
        const newCharId = charInsertResult.insertId;

        const eligibleRaids = allRaids
          .filter(raid => raid.entry_level <= itemLevel)
          .map(raid => ({ ...raid, totalGold: (raid.gold_per_gate || []).reduce((a, b) => a + b, 0) }))
          .sort((a, b) => b.totalGold - a.totalGold)
          .slice(0, 3);
        
        if (eligibleRaids.length > 0) {
            const raidsToInsert = eligibleRaids.map(raid => [
                newCharId, raid.id, JSON.stringify(Array(raid.gates).fill(false))
            ]);
            await connection.query('INSERT INTO character_raids (character_id, raid_id, gate_progress) VALUES ?', [raidsToInsert]);
        }
      }
    }
    ...
    res.status(500).json({ message: '회원가입 중 데이터베이스 오류가 발생했습니다.' });
  } finally {
    connection.release();
  }
});
...
app.get('/api/characters', verifyToken, async (req, res) => {
    try {
        const [characters] = await dbPool.query('SELECT * FROM characters WHERE user_id = ?', [req.userId]);
        
        const characterPromises = characters.map(async (char) => {
            const [raids] = await dbPool.query(
                `SELECT r.id, r.raid_name, r.difficulty_name, r.gold_per_gate, cr.gate_progress 
            ...
            const weeklyRaids = raids.map(raid => ({
                raidId: raid.id.split('_')[0],
                difficultyId: raid.id,
                name: raid.raid_name,
                difficultyName: raid.difficulty_name,
                // mysql2/promise가 JSON을 자동으로 파싱하므로 JSON.parse 제거
                gates: raid.gate_progress || [],
                gold: raid.gold_per_gate || []
            }));
            ...
            return {
                ...
                daily: { kurzanRest: char.kurzan_rest, guardianRest: char.guardian_rest, kurzanDone: false, guardianDone: false },
                memo: '',
                settings: { showWeekly: !!char.show_weekly, showDaily: !!char.show_daily, isGoldCharacter: !!char.is_gold_character, isHidden: !!char.is_hidden },
                weeklyRaids: weeklyRaids
            };
        });
...
app.get('/api/raids', async (req, res) => {
    try {
        ...
        raidsFromDB.forEach(raid => {
            const raidBaseName = raid.raid_name;
            // 'valtan_single' -> 'valtan'
            const raidBaseId = raid.id.substring(0, raid.id.indexOf('_')); 
            ...
            raidMap.get(raidBaseId).difficulties.push({
                ...
                gates: raid.gates,
                gold: raid.gold_per_gate,
                more_gold_per_gate: raid.more_gold_per_gate
            });
        });
